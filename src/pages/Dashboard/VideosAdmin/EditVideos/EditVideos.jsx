...
const EditVideos = () => {
  ...
  useEffect(() => {
    const fetchVideo = async () => {
      try {
        ...
        setCurrentImage(video.thumbnail || null);
      } catch (err) {
        console.error("Error fetching video:", err);
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Failed to load video data.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axiosInstance.put(`/videos/${id}`, videoData);
      ...
      Swal.fire({
        ...
        navigate(-1); // Redirect to videos admin after success
      });
    ...
    return (
      <div className="p-4 sm:p-6 max-w-4xl mx-auto min-h-screen">
        <div className="flex justify-center items-center h-64">
          ...
          <span className="ml-3 text-lg">Loading video...</span>
        </div>
      </div>
    );
  }
